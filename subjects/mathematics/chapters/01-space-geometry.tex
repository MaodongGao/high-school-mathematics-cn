...
\]
也就是说，方向余弦就是把方向向量化为单位向量后的三个坐标。由单位向量长度为 $1$，立即得到
\[
  \cos^2\alpha+\cos^2\beta+\cos^2\gamma=1,
  \qquad
  \sin^2\alpha+\sin^2\beta+\sin^2\gamma=2.
\]
第二式只是把 $\sin^2\theta=1-\cos^2\theta$ 对三个角相加，并不是另一条需要死记的公式。
...
\]
这是把所有侧面三角形展开后相加：每个三角形面积为“$\frac12\times$ 底边 $\times l$”，所有底边之和正好是底面周长 $p$。
正棱台上下底周长为 $p,p'$、斜高为 $l$，则
\[
  S_{\text{正棱台侧}}=\frac12(p+p')l.
\]
同理，每个侧面是梯形；把所有梯形面积相加，上、下底边之和分别累积成 $p'$ 与 $p$。
...
表面积公式都可以从展开图重新得到，不必孤立记忆：圆柱侧面展开为长 $2\pi r$、宽 $l$ 的矩形；圆锥侧面展开为半径 $l$ 的扇形，弧长等于底面周长 $2\pi r$；圆台可看成“大圆锥侧面减小圆锥侧面”。
...
记忆时不要把两式割裂：球的表面积等于半径为 $R$ 的大圆面积的 $4$ 倍；球的体积还可写成
\[
  V_{\text{球}}=\frac R3S_{\text{球}}.
\]
后一个关系可理解为把球面分成许多很小的面片，并把每个面片与球心连成高近似为 $R$ 的小棱锥；所有小棱锥体积相加即为球体积。
